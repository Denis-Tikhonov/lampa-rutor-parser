  ...
  async fetch(request) {
    ...
    try {
      ...
      const [rutorPages, nnmBuffer, xxxtorHtml, lepornoHtml] = await Promise.all([
        ...
        fetch(`https://leporno.de/search.php`, {
          ...
          body: new URLSearchParams({ 
            ...
            'terms': 'all', 
            'sr': 'topics', 
            'sf': 'titleonly', // Ищем только в заголовках для точности
            'submit': 'Search' 
          }).toString()
        }).then(r => r.text()).catch(() => "")
      ]);
      ...
      if (nnmBuffer) {
        ...
        await Promise.all(nnmItems.slice(0, 10).map(async it => {
          try {
      ...
      // ===================================================
      // 5. ОБНОВЛЕННЫЙ ПАРСИНГ LEPORNO.DE (ИСПОЛЬЗУЯ ТВОЙ ФРАГМЕНТ)
      // ===================================================
      if (lepornoHtml) {
        // Используем структуру из твоего фрагмента: tr valign="middle" + topictitle
        const rowRegex = /<tr\s+valign=["']middle["'][^>]*>([\s\S]*?)<\/tr>/gi;
        const lepItems = [];
        let rm;
        while ((rm = rowRegex.exec(lepornoHtml)) !== null) {
          const row = rm[1];
          // Ищем ссылку на топик внутри строки
          ...
          if (topicMatch) {
            ...
            lepItems.push({ id: topicId, title: title });
          }
          if (lepItems.length >= 15) break; // Ограничение для стабильности
        }

        debug.trackers.leporno = { foundTopics: lepItems.length };
        ...
        await Promise.all(lepItems.map(async item => {
          try {
            ...
            // Ищем ID торрента для скачивания
            const fileIdMatch = h.match(/download\/file\.php\?id=(\d+)/);
            if (fileIdMatch) {
              const fileId = fileIdMatch[1];
              
              // 1. Пытаемся получить магнет через доп. заголовок (если сайт позволяет)
              // 2. Если нет - вычисляем хеш из самого торрент-файла
              ...
              if (hash && !seen.has(hash)) {
                ...
                const peers = h.match(/class=["'][^"']*leech[^"']*["'][^>]*><b>(\d+)<\/b>/i);
...
// --- СЕРДЦЕ ПАРСЕРА: ВЫЧИСЛЕНИЕ INFO-HASH ---
