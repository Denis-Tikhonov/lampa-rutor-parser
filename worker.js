    ...
    try {
      ...
      for (const html of rutorPages) {
        ...
        while ((m = rowRegex.exec(html)) !== null) {
          ...
          Results.push({
            Title: t[2].trim(),
            Seeders: sd ? parseInt(sd[1]) : 0,
            Peers: lc ? parseInt(lc[1]) : 0,
            Size: sInfo ? parseSizeToBytes(sInfo[1], sInfo[2]) : 0,
            Tracker: "Rutor",
      ...
      if (lepornoHtml && lepornoHtml.length > 100) {
        const lepRowRegex = /<tr\s+valign=["']middle["'][^>]*>([\s\S]*?)<\/tr>/gi;
        const lepItems = [];
        let rm;

        while ((rm = lepRowRegex.exec(lepornoHtml)) !== null) {
          const row = rm[1];
...
async function getInfoHashFixed(buffer) {
  try {
    ...
    const infoKeyBytes = [52, 58, 105, 110, 102, 111];
    let infoIndex = -1;
...
function parseBencodeElement(data, pos) {
  ...
  if (byte >= 0x30 && byte <= 0x39) {
    ...
    if (pos >= data.length || data[pos] !== 0x3A) return -1;
    pos++;
