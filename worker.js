...
// =====================================================================
async function getInfoHashFixed(buffer) {
  try {
    const uint8 = new Uint8Array(buffer);

    // Ищем "4:info" побайтово
    const infoKeyBytes = [52, 58, 105, 110, 102, 111]; // "4:info"
    let infoIndex = -1;
    for (let i = 0; i < uint8.length - 6; i++) {
      if (uint8[i] === 52 && uint8[i+1] === 58 &&
          uint8[i+2] === 105 && uint8[i+3] === 110 &&
          uint8[i+4] === 102 && uint8[i+5] === 111) {
        infoIndex = i;
        break;
      }
    }
    ...
    const infoStart = infoIndex + 6; // после "4:info"

    // Парсим bencode-элемент начиная с infoStart чтобы найти его конец
    const infoEnd = parseBencodeElement(uint8, infoStart);
    if (infoEnd === -1) return null;

    // Вычисляем SHA-1 от info-словаря
...
/**
 * Парсит один bencode-элемент и возвращает позицию ПОСЛЕ него.
 ...
 *   NN:...  — строка (NN = длина)
 */
function parseBencodeElement(data, pos) {
  ...
  if (byte === 0x64) { // 'd'
    pos++; // пропускаем 'd'
    while (pos < data.length && data[pos] !== 0x65) { // не 'e'
      // ключ (всегда строка)
      pos = parseBencodeElement(data, pos);
      if (pos === -1) return -1;
      // значение
      pos = parseBencodeElement(data, pos);
      if (pos === -1) return -1;
  ...
  if (byte === 0x69) { // 'i'
    pos++; // пропускаем 'i'
    while (pos < data.length && data[pos] !== 0x65) { // ищем 'e'
      pos++;
    }
    if (pos >= data.length) return -1;
    return pos + 1; // пропускаем 'e'
  ...
  if (byte >= 0x30 && byte <= 0x39) { // '0'-'9'
    ...
    if (pos >= data.length || data[pos] !== 0x3A) return -1; // должно быть ':'
    pos++; // пропускаем ':'
    const strLen = parseInt(numStr, 10);
    if (isNaN(strLen)) return -1;
    return pos + strLen;
  }
